...
classdef Shapes < matlab.mixin.Heterogeneous & handle
   ...
   properties
       ...
       position % tâm của vật cản (3x1)
       velocity % vận tốc hiện tại (3x1)
       velocityFunction % hàm vận tốc: v = f(x,t)

       gamma % handle tới hàm gamma (chuẩn)
       gammaDistance % handle tới hàm gamma dựa khoảng cách (Task 4)
       gradientGamma % handle tới gradient của gamma (pháp tuyến)
       shapeParameters % struct lưu thông số riêng (vd. radius, a,b,c)

       rho % độ nhạy điều chế (Part 2)
       patchVal % handle đồ hoạ 'patch' để cập nhật hình vẽ
       prevTime % thời điểm trước đó (để tính ∆t khi move)
   end
       ...
       function self = Shapes(gamma, gammaDistance, gradientGamma, parameters, position, velocityFunction, rho)
           ...
           if nargin > 0
              ...
              if size(position, 2) == 3 
                position = position';  % ép về cột 3x1 nếu truyền hàng 1x3
              end
              % Save parameters
              ...
              self.shapeParameters = parameters; % vd parameters.radius
              self.prevTime = 0;
              %Công thức gamma giả định vật nằm ở gốc 000, mà truyền vào x, y
              %,z thì vật nằm ở bất cứ đâu nên phải trừ đi để như
              %thể mình đặt vật tính từ ở gốc cho tính toán chính xác
              ...
              self.gradientGamma = @(x, y, z) gradientGamma(x-self.position(1), ...
                                                            ...
                                                            z-self.position(3), ...
                                                            parameters);
             %điều chỉnh độ nhạy điều chế quanh vật cản
        ...
        function showShape(self, axisLimit, alpha)
            ...
            self.patchVal = patch(s, 'FaceColor', rand(1, 3), 'FaceAlpha', alpha, 'EdgeColor', 'none'); %lưu để có thể di chuyển
