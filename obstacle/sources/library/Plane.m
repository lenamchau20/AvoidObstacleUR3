...
classdef Plane < Shapes
   properties
      normal %vector pháp tuyến của mặt phẳng
   ...
   methods
       ...
       function self = Plane(normal, position, velocity_function, rho)
           ...
           param = []; %tạo struct
           param.normal = normal/norm(normal); %norm(normal) tính độ dài (Euclidean norm) của vector
                                               %->chuẩn hoá vector pháp tuyến thành vector đơn vị (độ dài = 1).
           ...
           gammaDistance = @(x, y, z, param) 1 + abs(param.normal(1)*x + param.normal(2)*y + param.normal(3)*z);

           %gradientGamma = @(x, y, z, param) sign(param.normal(1)*x + param.normal(2)*y + param.normal(3)*z) * param.normal(:);
           ...
           % for display
           gamma = @(x, y, z, param) param.normal(1)*x + param.normal(2)*y + param.normal(3)*z + 1; %PT MẶT PHẲNG: ax + by + cz + d = 0; (a,b,c) vecto pháp tuyến
