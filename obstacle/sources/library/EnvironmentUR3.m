...
classdef EnvironmentUR3 < handle
   properties
      listOfObstacles
      ...
      robot
      endEffectorRadius = 0.05
      endEffectorPosition
   end

   methods
    
       function self = EnvironmentUR3(robotPosition)

        filepath = fileparts(which('EnvironmentUR3.m'));
        ...
        self.robot = UR3Wrapper();
        show(self.robot.robot, zeros(6, 1), 'PreservePlot', false, 'Frames', 'off', 'Visuals', 'on');
    ...
    function self = showGammaIsosurfaces(self, isoValues, useDistance, color)  %dùng vẽ các mặt isosurface của hàm gamma trong 3D để quan sát vật cản
                                           %self:đối tượng môi trường (EnvironmentUR3) chứa danh sách vật cản
                                           %isoValues: mảng giá trị cần vẽ
                                           %useDistance:bật =1 để dùng gammaDistance, mặc định =0 dùng gamma.
        if exist('color', 'var') == false %nếu không tryền màu thì lấy màu mặc định 
            color = [0.2, 0, 0.8]; %[0.2, 0, 0.8];
        end

        if exist('useDistance', 'var') == false %nếu không truyền useDistance thì mặc định false = gamma
            useDistance = false;
        end
        %tạo khối 3d với mỗi trục 50 điểm
        nPoints = 50;
        [x,y,z] = meshgrid(linspace(self.axisLimit(1), self.axisLimit(2), nPoints), ...
                  ...
                  %axisLimit chứa giới hạn không gian: [xmin xmax ymin ymax zmin zmax]
        for obstacle = self.listOfObstacles %quét từng vật cản vì các obstacle đều lưu vào list

            for i=1:length(isoValues)
                value = isoValues(i);
                if useDistance %dùng gammaDistance
                    s = isosurface(x, y, z, obstacle.gammaDistance(x, y, z), value); %isosurface: hàm Matlab để ve mặt đẳng hướng
                else %dùng gamma
                    s = isosurface(x, y, z, obstacle.gamma(x, y, z), value);
                end
                alpha = 0.5 * (1.1-i/length(isoValues)); %độ trong suốt giảm dần theo lớp ngoài
    ...
    function self = addCylinder(self, radius, position, rho, velocity_function)
        ...
        self.listOfObstacles = [self.listOfObstacles, newCylinder]; %save obstacle in list

        % If position is inside obstacle, we count it as a boundary
        if newCylinder.gamma(self.endEffectorPosition(1), self.endEffectorPosition(2), self.endEffectorPosition(3)) > 1     %check EE whether inside or outside obstacle        
            alpha = 0.5; %transparency
        else 
            alpha = 0.1;
        end

        newCylinder.showShape(self.axisLimit, alpha); %draw shape
