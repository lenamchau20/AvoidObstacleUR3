...
classdef UR3Wrapper
   ...
   methods
       function self = UR3Wrapper()
           % Create and store 'ur3_arm' robot model
           ...
           % functions for the robot object.
            ...
            self.robot = importrobot(fullfile('ur3-model', 'ur3.urdf'), ...
                'MeshPath', fullfile('ur3-model', 'meshes', 'collision'));
       ...
       function cartesianTrajectory = computeForwardKinematics(self, jointTrajectory)
           ...
           % jointTrajectory is a 6xN matrix with the 6 joints in each column
           % cartesianTrajectory is a 3xN matrix with the 3D position in each column
           
           jointTrajectory = self.checkDimensions(jointTrajectory, 6);
           
           nTrajectory =  size(jointTrajectory, 2);

           cartesianTrajectory = nan(3, nTrajectory);
           for iTraj = 1:nTrajectory
               transform = self.robot.getTransform(jointTrajectory(:,iTraj), 'ee_link', 'base_link');
       ...
       function jointTrajectory = computeInverseKinematics(self, cartesianTrajectory, initialJoint, showBar, targetRotation)
           ...
           jointTrajectory = nan(6, nTrajectory);
           
           inverseKinematic = robotics.InverseKinematics('RigidBodyTree', self.robot);
            
           if showBar
               h = waitbar(0, 'Computing inverse kinematics...');
               h.Position(3:4) = [300, 100];
               for iTraj = 1:nTrajectory
                   jointTrajectory(:,iTraj) = inverseKinematic('ee_link', ...
       ...
       function cartesianVelocity = computeForwardVelocity(self, jointTrajectory, jointSpeed)
           ...
           % speed
           % jointTrajectory is a 6xN matrix with the 6 joints in each column
           % jointSpeed is a 6xN matrix with the 6 joints in each column
           % cartesianVelocity is a 3xN matrix with the 3D speed in each column
           
           jointTrajectory = self.checkDimensions(jointTrajectory, 6);
           jointSpeed = self.checkDimensions(jointSpeed, 6);
           
           nTrajectory =  size(jointTrajectory, 2);

           cartesianVelocity = nan(3, nTrajectory);
           for iTraj = 1:nTrajectory
               jacobian = self.robot.geometricJacobian(jointTrajectory(:,iTraj), 'ee_link');
       ...
       function jointTorque = computeInverseDynamic(self, jointTrajectory, jointSpeed, jointAcceleration)
            ...
            % jointTrajectory is a Nx6 or 6xN matrix
            % jointSpeed is a Nx6 or 6xN matrix
            % jointAcceleration is a Nx6 or 6xN matrix
            % jointTorque is a 6xN matrix with the 6 joints in each column     
           
            % Check dimensions of arguments
            jointTrajectory = self.checkDimensions(jointTrajectory, 6);
            jointSpeed = self.checkDimensions(jointSpeed, 6);
            jointAcceleration = self.checkDimensions(jointAcceleration, 6);
            
            nTrajectory =  size(jointTrajectory, 2);

            jointTorque = nan(6, nTrajectory);
            for iTraj = 1:nTrajectory
       ...
       function [simulatedTime, simulatedJoint, simulatedSpeed] = simulateRobot(self, optimalController, cartesianTarget)
           ...
           show(self.robot, optimalController.currentSolution.Xopt(1:6, 1), ...
           ...
           overSampling = 20;
           simulatedJoint = nan(6, overSampling*(nTrajectory-1));
           currentJoint = optimalController.currentSolution.Xopt(1:6, 1);
           simulatedSpeed = nan(6, overSampling*(nTrajectory-1));
           currentSpeed = optimalController.currentSolution.Xopt(7:12, 1);
           simulatedTime = nan(1, overSampling*(nTrajectory-1));

           optimalController.perturbations = zeros(6, 1);
           ...
           for iTraj=2:nTrajectory
               ...
               currentJoint = y(end,1:6)';
               currentSpeed = y(end,7:12)';
               ...
               simulatedJoint(:,(iTraj-2)*overSampling+1:(iTraj-1)*overSampling) = interp1(t, y(:,1:6), sampletime)';
               simulatedSpeed(:,(iTraj-2)*overSampling+1:(iTraj-1)*overSampling) = interp1(t, y(:,7:12), sampletime)'; 
...
function dydt = forwardStateDynamic(t, y, optimalController)
    ...
    dydt = zeros(12, 1);
    dydt(1:6) = jointSpeed;
    dydt(7:12) = optimalController.robot.robot.forwardDynamics(jointPosition, jointSpeed, controlTorque) ...
...
function keyPressCallback(source, eventdata, control, currentJoint)
      ...
      jacobian = control.robot.robot.geometricJacobian(targetPosition, 'ee_link');
      newPerturbation = zeros(6, 1);
